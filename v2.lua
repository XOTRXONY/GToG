...
FloatingButton.Parent = SimpleSpy2
FloatingButton.Size = UDim2.new(0, 50, 0, 50)
FloatingButton.Position = UDim2.new(0, 10, 0, 100)
FloatingButton.BackgroundColor3 = Color3.fromRGB(80, 80, 220)
...
FloatingButtonCorner.Name = "UICorner"
FloatingButtonCorner.CornerRadius = UDim.new(0.5, 0)
...
function genScript(remote, args)
	...
	gen = "-- credits to exx#9394 Mob du8\n\n" .. gen
	prevTables = {}
	return gen
end
...
function makeDraggable(elementToDrag, gripElement)
	local isDragging = false
	local inputObject = nil
	local dragStart = nil
	local startPosition = nil

	gripElement.InputBegan:Connect(function(input)
		if
		...
		then
			...
			startPosition = elementToDrag.AbsolutePosition
		end
	end)

	UserInputService.InputChanged:Connect(function(input)
		if isDragging and input == inputObject then
			local newPosition = startPosition + (input.Position - dragStart)
			elementToDrag.Position = UDim2.new(0, newPosition.X, 0, newPosition.Y)
		end
	end)
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		...
		FloatingButton.MouseButton1Click:Connect(function()
			...
			Background.Visible = isMainUIVisible
		end)
